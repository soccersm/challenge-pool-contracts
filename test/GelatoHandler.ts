...
import { GelatoHandler__factory } from "../typechain-types";
  ...
  it("Should revert for createChallengeRelay: multi", async function() {
    ...
    //multi event
     const multiCorrectScoreChallenge = multiCorrectScore(
        ...
        await expect((gelatoHandlerProxy.connect(baller)as any).createChallengeRelay(...preparedMultiCorrectScoreChallenge)).to.not.be.reverted;
